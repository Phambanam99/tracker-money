...
class RoomRepositoryImpl : RoomRepository {
    ...
    override suspend fun findByCode(code: String): Room? = dbQuery {
        RoomsTable.selectAll().where { RoomsTable.code eq code }.map { it.toRoom() }.singleOrNull()
    }

    override suspend fun findById(id: String): Room? = dbQuery {
        RoomsTable.selectAll().where { RoomsTable.id eq id }.map { it.toRoom() }.singleOrNull()
    }
